...
class Forwarder(object):
    ...
    def _forward(self, data_wrapper, batch_size = 512):
        ...
        for data in dataloader:
            ...
            log_probs[offset : offset + output.shape[1], :] = output.data.cpu()
...
class Trainer(Forwarder):
    ...
    def train(self, sequence, transcript, batch_size = 512, learning_rate = 0.1):
        ...
        # update the labels of the sequence in the buffer
        data_wrapper.labels = labels[0:] # deep copy
        # sample buffered frames
        n_old_frames = min( int(sequence.shape[1] * self.buffered_frame_ratio), self.buffer.n_frames() )
        for _ in range(n_old_frames):
            seq_idx, frame_idx = self.buffer.random()
            data_wrapper.add_buffered_frame(self.buffer.features[seq_idx], self.buffer.framelabels[seq_idx][frame_idx], frame_idx)
        # train model on Viterbi decoded sequence and some buffered frames
        dataloader = torch.utils.data.DataLoader(data_wrapper, batch_size = batch_size, shuffle = True)
